...
이 문서는 Source of Truth가 아니다. `docs/CONTENT_GUIDE.md` §13(Content Source Rule)·§14(Traceability Rule)를 개별 프로젝트에 적용하기 위한 범용 템플릿이며, `data/projects.json`에는 반영되지 않는다 (`docs/ARCHITECTURE.md` §6, §11 Project Identifier Rule).

**사용 방법**: 실제 프로젝트가 확정되고 `slug`(`docs/DATA_MODEL.md` §5.1)가 결정되면, 이 파일을 `docs/projects/<slug>/evidence.md`로 복사한 뒤 제목과 §3의 "출처 상세"를 실제 프로젝트 자료로 채운다. 폴더/파일 경로가 왜 slug를 그대로 쓰는지는 `docs/ARCHITECTURE.md` §11(Project Identifier Rule)을 따른다 — 프로젝트명이 확정되기 전까지는 이 템플릿 자체를 특정 프로젝트에 종속시키지 않는다.
...
실제 프로젝트가 확정되면 이 템플릿을 `docs/projects/<slug>/evidence.md`로 복사해 사용한다. §4.2의 최소 조건이 충족되면 `feature/projects-<slug>-content` 브랜치에서 Content Workflow(§11) 2단계(초안 작성)부터 진행한다.
